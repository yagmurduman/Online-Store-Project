...
<template>
  ...
  <div class="container">   
    ...
    <!--Section: Content-->
    <section  style=" box-shadow: 0 0 20px rgb(124, 85, 75);" class="row-fluid justify-content-start">
  ...
  <div class=" col mt-5 ">
    ...
    <h3 class="font-size-lg font-weight-bold text-center dark-grey-text mb-5 mt-5">{{this.$route.params.name}}</h3>
...
export default {
  ...
  methods:{
    ...
    loadmore() {
      ...
      if(this.filtered==0){
         console.log("no")
          axios.post('http://localhost:8090/product/get-by-category=' + this.$route.params.id + "?key="+ this.len)
          .then(response => {
    ...
    filter(){
    console.log("http://localhost:8090/filter/extended-filter?category="+ this.$route.params.id+"&orderType="+this.order+"&price="+this.price+"&rate="+this.rate+"&type="+this.$route.params.type)
    axios.post(
       //ilk key 0
      "http://localhost:8090/filter/extended-filter?category="+ this.$route.params.id+"&key=0&orderType="+this.order+"&price="+this.price+"&rate="+this.rate+"&type="+this.$route.params.type)    
    ...
    filterrest(){
       axios.post(
       //ilk key 0
      "http://localhost:8090/filter/extended-filter?category="+this.$route.params.id+"&key="+this.len+"+&orderType="+this.order+"&price="+this.price+"&rate="+this.rate+"&type="+this.$route.params.type)    
     .then(response => {
            {if(response.status==200)
   ...
   updated(){  
    ...
    mounted(){
      ...
      axios.post('http://localhost:8090/product/get-by-category=' + this.$route.params.id + "?key=0")
      .then(response => (this.products = response.data))
      .catch(() => console.log('category.vue')); 
    ...
    beforeRouteUpdate (to, from, next) {
      ...
      axios.post('http://localhost:8090/product/get-by-category=' + to.params.id + "?key=0")
      .then(response => (this.products = response.data))
      .catch(() => console.log('category.vue')); 
      next();
  }  
