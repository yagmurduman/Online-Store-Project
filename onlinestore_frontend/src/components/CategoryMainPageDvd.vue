<template>
  <div class="container">
    ...
    <img   class="container rounded" style=" box-shadow: 0 0 10px rgb(124, 85, 75);"  src="../assets/Films.png" alt="" />
      ...
      <div class="col-lg-2 border mx-auto">
          ...
          <div class="mb-5">
            ...
            <div v-for="c in cat" :key="c.catId" class="form-check pl-0 mb-2">
              <input type="radio"  v-model="category" :value="c.catId" class="form-check-input" id="materialGroupExample2" name="groupOfMaterialRadios2">
              <label class="form-check-label" for="materialGroupExample2">{{c.catName}}</label>
            </div>
...
export default {
  ...
  methods:{
    ...
    filteredchange2(){
     ...
     loadmore() {
        if(this.filtered==0){
         console.log("no")
          axios.post("http://localhost:8090/product/get-by-type/?key="+this.len +"&type=dvd" )
          .then(response => {
    ...
    filter(){
     ...
     axios.post("http://localhost:8090/filter/extended-filter?category="+this.category+"&key=0&orderType="+this.order+"&price="+this.price+"&rate="+this.rate+"&type=dvd")    
   ...
   filterrest(){
       axios.post(
       //ilk key 0
      "http://localhost:8090/filter/extended-filter?category="+this.category+"&key="+this.len+"+&orderType="+this.order+"&price="+this.price+"&rate="+this.rate+"&type=dvd")    
     .then(response => {
            {if(response.status==200)
  ...
  mounted(){
    axios.post("http://localhost:8090/product/get-by-type/?key=0&type=dvd")
    .then(response=> this.products=response.data)
